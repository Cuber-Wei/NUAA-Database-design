            ...
            for hash_value, identical_files in hash_groups.items():
                if saved_count == 0:
                    ...
                    try:
                        ...
                        saved_count += 1
                        print(
                            f"  ✓ 保留: {best_file['original_filename']} -> {new_filename} (大小: {best_file['size']} bytes)"
                        )
                    except Exception as e:
                        print(f"  ✗ 复制文件失败 {best_file['original_filename']}: {e}")

                # 统计重复文件
                duplicates_removed += len(identical_files) - (
                    1 if saved_count <= 1 else 0
                )
                ...
                for file_info in identical_files:
                    if saved_count <= 1 and file_info != best_file:
                        print(
                            f"  - 删除重复: {file_info['original_filename']} (大小: {file_info['size']} bytes)"
                        )
                    elif saved_count > 1:
                        print(
                            f"  - 删除重复: {file_info['original_filename']} (大小: {file_info['size']} bytes)"
